                   ...
                   newcolname = "wearing")
{
    ...
    # Creates a vector of booleans where the i'th value is true iff the value
    # in ct at the i'th index is greater than 0. Important to note that the
    # vector sizes still match.
    # e.g.
    # ct = [1, 5, 3, 0, 0, 2, 1, 0]
    # ct_bool = [TRUE, TRUE, TRUE, FALSE, FALSE, TRUE, TRUE, FALSE]
    ct_bool = ct > 0

    # Returns a list of the nonzero index locations
    # [1, 3, 0, 5, 0, 0, 1] would return
    # [1, 2, 4, 7]
    ...
    # Find the size of each window of nonzero values
    # let endpos = [1, 6, 9, 12]
    #     startpos = [3, 7, 11, 15]
    # then
    #     allowancewin = [2, 1, 2, 3]
